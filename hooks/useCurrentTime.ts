 ...
 * should use this. The polling is paused when the component is unmounted.
 *
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (now - lastUpdateRef.current >= intervalMs) {
        ...
        // Only trigger a React re-render if the value actually changed (integer comparison)
        setTime(prev => {
          // For progress bar: compare at ~0.25s granularity (fps=4 → 250ms)
          // For lyrics: compare at ~0.1s granularity (fps=10 → 100ms)
